...
const MAX_SUM_TER = 5e8;
const MAX_SUM_CENT = 5e8;
...
class AccountApp extends require("./dapp")
{
    ...
    Start(bClean)
    {
        ...
        this._DBStateWrite({Num:0, PubKey:[], Value:{BlockNum:1, SumCOIN:1 * TOTAL_SUPPLY_TERA}, Name:"System account"}, 1)
        for(var i = 1; i < 8; i++)
            this._DBStateWrite({Num:i, PubKey:[], Value:{BlockNum:1}, Name:""})
        this._DBStateWrite({Num:8, PubKey:GetArrFromHex(ARR_PUB_KEY[0]), Value:{BlockNum:1, SumCOIN:0}, Name:"Founder account"})
